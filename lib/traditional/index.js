    ...
    return {
        'upload': function(req, res, next){
            ...
            if (req.body[params.chunkIndex]){
                ...
                debug('simple upload');
                storage.store_file(req, uploadedFile).then(function(){
                    res.end(respond());
        ...
        'delete': function(req, res, next){
            ...
            storage.delete_file(req, deleteParams.uuid).then(function(){
                res.statusCode = 200;
        ...
        'deleteall':function (req, res, next) {
            storage.deleteall(req).then(function () {
                res.statusCode = 200;
                res.send(JSON.stringify(respond()));
            }, function (err) {
                res.statusCode = 500;
                res.send(JSON.string(respond(err)));
            });

            return res.end();
        },
        'getall': function (req, res, next) {
            storage.getall(req).then(function (result) {
                ...
                //debug("getall exec failed.");
                res.statusCode = 500;
                res.send(JSON.stringify(respond(err)));
                res.end();
            });
        },
        'jsongetall': function(req, res, next){
             storage.getall(req).then(function (result) {
                //debug("result is: "+ JSON.stringify(result));
                res.status(200).type('json').json({'result':result}).end();
            }, function (err) {
                //debug("getall exec failed.");
                res.status(500).type('json').json(respond(err)).end();;
            });           
        },
